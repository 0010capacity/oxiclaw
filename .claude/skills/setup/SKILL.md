---
name: setup
description: Run initial OxiClaw setup. Use when user wants to install dependencies, authenticate messaging channels, register their main channel, or start the background services. Triggers on "setup", "install", "configure oxiclaw", or first-time setup requests.
---
...
**Case A — `origin` points to `qwibitai/oxiclaw` (user cloned directly):**

The user cloned instead of forking. AskUserQuestion: "You cloned OxiClaw directly. We recommend forking so you can push your customizations. Would you like to set up a fork?"
- Fork now (recommended) — walk them through it
- Continue without fork — they'll only have local changes

If fork: instruct the user to fork `qwibitai/oxiclaw` on GitHub (they need to do this in their browser), then ask them for their GitHub username. Run:
```bash
git remote rename origin upstream
git remote add origin https://github.com/<their-username>/oxiclaw.git
...
```bash
git remote add upstream https://github.com/qwibitai/oxiclaw.git
```
...
**Verify:** `git remote -v` should show `origin` → user's repo, `upstream` → `qwibitai/oxiclaw.git`.
...
2. **Fresh start** — "Skip migration and set up OxiClaw from scratch."
...
```bash
grep -q "getRuntimeBin" src/container-runtime.ts && echo "ALREADY_CONVERTED" || echo "NEEDS_CONVERSION"
```
...
- macOS: `launchctl unload ~/Library/LaunchAgents/com.oxiclaw.plist`
- Linux: `systemctl --user stop oxiclaw` (or `systemctl stop oxiclaw` if root)

Run `npx tsx setup/index.ts --step service` and parse the status block.

**If FALLBACK=wsl_no_systemd:** WSL without systemd detected. Tell user they can either enable systemd in WSL (`echo -e "[boot]\nsystemd=true" | sudo tee /etc/wsl.conf` then restart WSL) or use the generated `start-oxiclaw.sh` wrapper.
...
- macOS: check `launchctl list | grep oxiclaw`. If PID=`-` and status non-zero, read `logs/oxiclaw.error.log`.
- Linux: check `systemctl --user status oxiclaw`.
...
- SERVICE=stopped → `npm run build`, then restart: `launchctl kickstart -k gui/$(id -u)/com.oxiclaw` (macOS) or `systemctl --user restart oxiclaw` (Linux) or `bash start-oxiclaw.sh` (WSL nohup)
- SERVICE=not_found → re-run step 7
- CREDENTIALS=missing → re-run step 4 (check `data/credentials/main/.pi/agent/auth.json`)
- CHANNEL_AUTH shows `not_found` for any channel → re-invoke that channel's skill (e.g. `/add-telegram`)
- REGISTERED_GROUPS=0 → re-invoke the channel skills from step 5
- MOUNT_ALLOWLIST=missing → `npx tsx setup/index.ts --step mounts -- --empty`

Tell user to test: send a message in their registered chat. Show: `tail -f logs/oxiclaw.log`

## 9. Optional Skills

After setup completes, offer optional skills based on context.

Run: `npx tsx setup/index.ts --step skills` and parse the status block.

The step outputs `RECOMMENDATIONS` and `RECOMMENDATION_COUNT`.

AskUserQuestion (multiSelect) with the detected channels as context:
...
- **/add-gmail** — "Read and send emails via Gmail"
- **/add-emacs** — "Open files in Emacs, manage org-mode tasks"
- **/add-parallel** — "Run multiple Claude agents in parallel"
- **/add-ollama-tool** — "Query local Ollama models"
- **/claw** — "Quick CLI commands for common tasks"

For each selected skill: invoke it with its skill name (e.g., invoke `/add-compact`).

**If no skills selected:** Run `npx tsx setup/index.ts --step skills -- --skip-offer`
