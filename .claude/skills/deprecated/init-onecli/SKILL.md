...
deprecated: true
note: Replaced by pi-mono SDK native credential management. No action needed.
---

# Initialize OneCLI Agent Vault (DEPRECATED)

> **This skill is deprecated.** OxiClaw now uses the pi-mono SDK with per-group `auth.json` files. The OneCLI-based credential system has been removed. No action is needed.
