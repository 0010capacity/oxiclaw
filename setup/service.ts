...
<dict>
    ...
    <string>${projectRoot}/logs/oxiclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/oxiclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.oxiclaw');
  } catch {
...
/**
 * Kill any orphaned oxiclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned oxiclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/oxiclaw.service';
    ...
    unitPath = path.join(unitDir, 'oxiclaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=OxiClaw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/oxiclaw.log
StandardError=append:${projectRoot}/logs/oxiclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned oxiclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable oxiclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start oxiclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active oxiclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-oxiclaw.sh');
  const pidFile = path.join(projectRoot, 'oxiclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-oxiclaw.sh — Start OxiClaw without systemd',
    ...
    '    echo "Stopping existing OxiClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting OxiClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/oxiclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/oxiclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "OxiClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/oxiclaw.log"`,
  ];
