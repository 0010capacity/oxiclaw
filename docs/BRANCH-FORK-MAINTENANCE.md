...
**`qwibitai/oxiclaw`** (upstream) — core engine with skill definitions (`.claude/skills/`). No channel code on `main`.

**Channel forks** (`oxiclaw-whatsapp`, `oxiclaw-telegram`, `oxiclaw-slack`, etc.) — each fork = upstream + one channel's code applied. Users clone upstream, then merge a fork into their clone to add a channel.
...
user clones upstream main
  ├── merges oxiclaw-whatsapp fork  → adds WhatsApp
...
```bash
# In your local oxiclaw checkout
git checkout main && git pull

# For a fork:
git fetch oxiclaw-whatsapp
git checkout -B whatsapp-merge oxiclaw-whatsapp/main
git merge main
...
git push oxiclaw-whatsapp HEAD:main
...
1. Fork `oxiclaw` to `oxiclaw-{channel}`
