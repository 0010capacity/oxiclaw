...
1. **Check for existing work.** Search open PRs and issues before starting:
   ```bash
   gh pr list --repo qwibitai/oxiclaw --search "<your feature>"
   gh issue list --repo qwibitai/oxiclaw --search "<your feature>"
   ```
...
OxiClaw uses [Claude Code skills](https://code.claude.com/docs/en/skills) — markdown files with optional supporting files that teach Claude how to do something. There are four types of skills in OxiClaw, each serving a different purpose.
...
Add capabilities to OxiClaw by merging a git branch. The SKILL.md contains setup instructions; the actual code lives on a `skill/*` branch.
...
1. Fork `qwibitai/oxiclaw` and branch from `main`
